...
local CONTOUR_GRADIENT = {
    { lo = 0.00, hi = 0.30, r = 15, g = 40,  b = 90,  a = 50 },  -- 最深
...
DEPTH_BANDS = {
    { lo = 0.00, hi = 0.30, index = 1, name = "深海区", depthMin = 40, depthMax = 999, r = 15, g = 40,  b = 90  },
    { lo = 0.30, hi = 0.42, index = 2, name = "次深区", depthMin = 25, depthMax = 40,  r = 25, g = 60,  b = 120 },
    { lo = 0.42, hi = 0.54, index = 3, name = "中层区", depthMin = 15, depthMax = 25,  r = 38, g = 85,  b = 150 },
    { lo = 0.54, hi = 0.66, index = 4, name = "浅层区", depthMin = 5,  depthMax = 15,  r = 50, g = 110, b = 175 },
    { lo = 0.66, hi = 1.01, index = 5, name = "近岸区", depthMin = 0,  depthMax = 5,   r = 65, g = 140, b = 200 },
}
...
local function DrawContourLines(wMinX, wMinY, wW, wH, gridN, w2s, bilinear)
    ...
    if bilinear then
        ...
        for gy = 0, gridN - 1 do
            for gx = 0, gridN - 1 do
                ...
                local pTop = nvgLinearGradient(vg, sx0, sy0, sx1, sy0,
                    nvgRGBA(r00,g00,b00,a00), nvgRGBA(r10,g10,b10,a10))
                nvgBeginPath(vg); nvgRect(vg, sx0, sy0, sw, sh*0.5)
                nvgFillPaint(vg, pTop); nvgFill(vg)

                local pBot = nvgLinearGradient(vg, sx0, sy0, sx1, sy0,
                    nvgRGBA(r01,g01,b01,a01), nvgRGBA(r11,g11,b11,a11))
                nvgBeginPath(vg); nvgRect(vg, sx0, sy0+sh*0.5, sw, sh*0.5)
                nvgFillPaint(vg, pBot); nvgFill(vg)
                ...
                local pV = nvgLinearGradient(vg, sx0, sy0, sx0, sy1,
                    nvgRGBA(rT,gT,bT,math.floor(aT*0.5+0.5)),
                    nvgRGBA(rB,gB,bB,math.floor(aB*0.5+0.5)))
                nvgBeginPath(vg); nvgRect(vg, sx0, sy0, sw, sh)
                nvgFillPaint(vg, pV); nvgFill(vg)
            end
        end
    else
            ...
            for gx = 0, gridN - 1 do
                ...
                for _, band in ipairs(CONTOUR_GRADIENT) do
                    if avg >= band.lo and avg < band.hi then
                        ...
                        local sx1, sy1 = w2s(wx0 + stepW, wy0 + stepH)
                        nvgBeginPath(vg)
                        nvgRect(vg, sx0, sy0, sx1-sx0, sy1-sy0)
                        nvgFillColor(vg, nvgRGBA(band.r, band.g, band.b, band.a))
                        nvgFill(vg)
                        break
                    end
                end
    ...
    for _, th in ipairs(CONTOUR_THRESHOLDS) do
        ...
        nvgStrokeColor(vg, nvgRGBA(th.r, th.g, th.b, th.a))
...
local ROD_TYPES = {
    {
        ...
        name = "溪钓竿",
        desc = "≤1g 小型鱼",
        lineStrength = 0.8,    -- 极细线，只能拉小型鱼
        stretchMax   = 6.0,    -- 软线，弹性大
        -- 竹绿色
...
local FISH_TYPES = {
    -- 翻车鱼
    { name="翻车鱼", diff=5,
      wMin=150, wMax=2300, wBias=0, wSpread=0.5,
    ...
    -- 蓝鳍金枪鱼
    { name="蓝鳍金枪鱼", diff=5,
      wMin=30, wMax=680, wBias=0.2, wSpread=0.27,
    ...
    -- 大王乌贼
    { name="大王乌贼", diff=5,
      wMin=37.5, wMax=500, wBias=0.2, wSpread=0.27,
    ...
    -- 长尾鲨
    { name="长尾鲨", diff=5,
      wMin=6, wMax=240, wBias=0.2, wSpread=0.27,
    ...
    -- 巨石斑鱼
    { name="巨石斑鱼", diff=5,
      wMin=7.5, wMax=300, wBias=0.2, wSpread=0.27,
    ...
    -- 魔鬼鱼
    { name="魔鬼鱼", diff=5,
      wMin=15, wMax=300, wBias=0.2, wSpread=0.27,
      forceAtMax=300, forceExp=0.45,
      tanCoeff=1, radSpeedMax=130, tanSpeedMax=600,
      radialStateTable = {
          { weight=0.9, label="calm", ampLo=0.1, ampHi=0.3, durMin=1, durMax=4 },
    ...
    -- 皇带鱼
    { name="皇带鱼", diff=5,
      wMin=15, wMax=270, wBias=0.2, wSpread=0.27,
    ...
    -- 鼠鲨
    { name="鼠鲨", diff=5,
      wMin=18, wMax=250, wBias=0.2, wSpread=0.27,
    ...
    -- 青鲨
    { name="青鲨", diff=4,
      wMin=12, wMax=200, wBias=0.2, wSpread=0.27,
    ...
    -- 旗鱼
    { name="旗鱼", diff=4,
      wMin=12, wMax=150, wBias=0.2, wSpread=0.27,
    ...
    -- 皱筛鲨
    { name="皱筛鲨", diff=4,
      wMin=6, wMax=100, wBias=0.2, wSpread=0.27,
    ...
    -- 刺魟
    { name="刺魟", diff=4,
      wMin=4.5, wMax=100, wBias=0.2, wSpread=0.27,
      forceAtMax=100, forceExp=0.45,
      tanCoeff=0.5, radSpeedMax=60, tanSpeedMax=600,
      radialStateTable = {
    ...
    -- 牛港鲹
    { name="牛港鲹", diff=4,
      wMin=2.25, wMax=80, wBias=0.2, wSpread=0.27,
    ...
    -- 犁头鳐
    { name="犁头鳐", diff=4,
      wMin=3, wMax=70, wBias=0.2, wSpread=0.27,
    ...
    -- 长鳍金枪鱼
    { name="长鳍金枪鱼", diff=4,
      wMin=5.25, wMax=70, wBias=0.2, wSpread=0.27,
      forceAtMax=70, forceExp=0.45,
    ...
    -- 月鱼
    { name="月鱼", diff=4,
      wMin=3, wMax=200, wBias=0.2, wSpread=0.27,
      forceAtMax=200, forceExp=0.45,
      tanCoeff=1, radSpeedMax=60, tanSpeedMax=600,
      radialStateTable = {
          { weight=0.9, label="calm", ampLo=0.1, ampHi=0.3, durMin=2, durMax=7 },
    ...
    -- 大海鲢
    { name="大海鲢", diff=4,
      wMin=2.25, wMax=145, wBias=0.2, wSpread=0.27,
    ...
    -- 高体鰤
    { name="高体鰤", diff=4,
      wMin=1.2, wMax=72, wBias=0.2, wSpread=0.27,
    ...
    -- 红尾梭鱼
    { name="红尾梭鱼", diff=1,
      wMin=0.09, wMax=1.5, wBias=0.2, wSpread=0.27,
    ...
    -- 真蛸
    { name="真蛸", diff=1,
      wMin=0.3, wMax=5, wBias=0.2, wSpread=0.27,
    ...
    -- 裸胸鳝
    { name="裸胸鳝", diff=3,
      wMin=0.45, wMax=24, wBias=0.2, wSpread=0.27,
    ...
    -- 海狼鱼
    { name="海狼鱼", diff=3,
      wMin=0.75, wMax=45, wBias=0.2, wSpread=0.27,
    ...
    -- 海鲇
    { name="海鲇", diff=2,
      wMin=0.45, wMax=20, wBias=0.2, wSpread=0.27,
    ...
    -- 马鲛鱼
    { name="马鲛鱼", diff=4,
      wMin=1.8, wMax=140, wBias=0.2, wSpread=0.27,
    ...
    -- 真鲷
    { name="真鲷", diff=2,
      wMin=0.15, wMax=10, wBias=0.2, wSpread=0.27,
    ...
    -- 东星斑
    { name="东星斑", diff=2,
      wMin=0.375, wMax=20, wBias=0.2, wSpread=0.27,
      forceAtMax=20, forceExp=0.45,
      tanCoeff=0.5, radSpeedMax=60, dragScale=0.6667, tanSpeedMax=600,
      radialStateTable = {
    ...
    -- 牙鲆
    { name="牙鲆", diff=2,
      wMin=0.225, wMax=10, wBias=0.2, wSpread=0.27,
      forceAtMax=10, forceExp=0.45,
    ...
    -- 金眼鲈
    { name="金眼鲈", diff=3,
      wMin=1.8, wMax=50, wBias=0.2, wSpread=0.27,
    ...
    -- 河豚
    { name="河豚", diff=1,
      wMin=0.15, wMax=3, wBias=0.2, wSpread=0.27,
    ...
    -- 黑鲷
    { name="黑鲷", diff=2,
      wMin=0.12, wMax=6, wBias=0.2, wSpread=0.27,
    ...
    -- 带鱼
    { name="带鱼", diff=2,
      wMin=0.3, wMax=12, wBias=0.2, wSpread=0.27,
    ...
    -- 鲈鱼
    { name="鲈鱼", diff=2,
      wMin=0.255, wMax=7, wBias=0.2, wSpread=0.27,
    ...
    -- 椰子海螺
    { name="椰子海螺", diff=1,
      wMin=0.15, wMax=2.5, wBias=0.2, wSpread=0.27,
    ...
    -- 金鲳鱼
    { name="金鲳鱼", diff=3,
      wMin=0.6, wMax=25, wBias=0.2, wSpread=0.27,
    ...
    -- 帆蜥
    { name="帆蜥", diff=3,
      wMin=0.6, wMax=22, wBias=0.2, wSpread=0.27,
    ...
    -- 鹦鹉螺
    { name="鹦鹉螺", diff=1,
      wMin=0.12, wMax=1.5, wBias=0.2, wSpread=0.27,
      forceAtMax=1.5, forceExp=0.45,
    ...
    -- 鼠尾鳕
    { name="鼠尾鳕", diff=1,
      wMin=0.15, wMax=5, wBias=0.2, wSpread=0.27,
      forceAtMax=5, forceExp=0.45,
    ...
    -- 褐拟鳞鲀
    { name="褐拟鳞鲀", diff=2,
      wMin=0.9, wMax=20, wBias=0.2, wSpread=0.27,
      forceAtMax=20, forceExp=0.45,
    ...
    -- 枪鱿
    { name="枪鱿", diff=1,
      wMin=0.09, wMax=4, wBias=0.2, wSpread=0.27,
    ...
    -- 香蕉鱼
    { name="香蕉鱼", diff=1,
      wMin=0.045, wMax=0.8, wBias=0.2, wSpread=0.27,
    ...
    -- 鲭鱼
    { name="鲭鱼", diff=1,
      wMin=0.09, wMax=1.4, wBias=0.2, wSpread=0.27,
    ...
    -- 鲍
    { name="鲍", diff=1,
      wMin=0.022, wMax=0.5, wBias=0.2, wSpread=0.27,
    ...
    -- 刺尻鱼
    { name="刺尻鱼", diff=1,
      wMin=0.03, wMax=0.5, wBias=0.2, wSpread=0.27,
      forceAtMax=0.5, forceExp=0.45,
    ...
    -- 竹荚鱼
    { name="竹荚鱼", diff=1,
      wMin=0.09, wMax=2.4, wBias=0.2, wSpread=0.27,
    ...
    -- 马夫鱼
    { name="马夫鱼", diff=1,
      wMin=0.022, wMax=0.6, wBias=0.2, wSpread=0.27,
    ...
    -- 红海星
    { name="红海星", diff=1,
      wMin=0.022, wMax=0.4, wBias=0.2, wSpread=0.27,
    ...
    -- 黑蝴蝶鱼
    { name="黑蝴蝶鱼", diff=1,
      wMin=0.018, wMax=0.35, wBias=0.2, wSpread=0.27,
    ...
    -- 蓝指海星
    { name="蓝指海星", diff=1,
      wMin=0.015, wMax=0.3, wBias=0.2, wSpread=0.27,
    ...
    -- 拟花鮨
    { name="拟花鮨", diff=1,
      wMin=0.022, wMax=0.3, wBias=0.2, wSpread=0.27,
      forceAtMax=0.3, forceExp=0.45,
    ...
    -- 蓝圆鲹
    { name="蓝圆鲹", diff=2,
      wMin=0.3, wMax=7, wBias=0.2, wSpread=0.27,
      forceAtMax=7, forceExp=0.45,
    ...
    -- 小丑鱼（伯爵）
    { name="小丑鱼（伯爵）", diff=1,
      wMin=0.003, wMax=0.07, wBias=0.2, wSpread=0.27,
    ...
    -- 紫海胆
    { name="紫海胆", diff=1,
      wMin=0.003, wMax=0.25, wBias=0.2, wSpread=0.27,
    ...
    -- 大鳍弹涂鱼
    { name="大鳍弹涂鱼", diff=1,
      wMin=0.012, wMax=0.15, wBias=0.2, wSpread=0.27,
    ...
    -- 文蛤
    { name="文蛤", diff=1,
      wMin=0.007, wMax=0.15, wBias=0.2, wSpread=0.27,
      forceAtMax=0.15, forceExp=0.45,
      tanCoeff=0.1, radSpeedMax=15, dragScale=0.02, tanSpeedMax=600,
      radialStateTable = {
          { weight=1, label="calm", ampLo=0.1, ampHi=0.3, durMin=1, durMax=7 },
    ...
    -- 蛤蜊
    { name="蛤蜊", diff=1,
      wMin=0.004, wMax=0.1, wBias=0.2, wSpread=0.27,
    ...
    -- 弹涂鱼
    { name="弹涂鱼", diff=1,
      wMin=0.007, wMax=0.1, wBias=0.2, wSpread=0.27,
      forceAtMax=0.1, forceExp=0.45,
      tanCoeff=0.1, radSpeedMax=15, dragScale=0.01333, tanSpeedMax=600,
      radialStateTable = {
          { weight=0.9, label="calm", ampLo=0.1, ampHi=0.3, durMin=1, durMax=7 },
    ...
    -- 沙丁鱼
    { name="沙丁鱼", diff=1,
      wMin=0.006, wMax=0.15, wBias=0.2, wSpread=0.27,
      forceAtMax=0.15, forceExp=0.45,
    ...
    -- 鞭冠鱼
    { name="鞭冠鱼", diff=2,
      wMin=0.3, wMax=6, wBias=0.2, wSpread=0.27,
      forceAtMax=6, forceExp=0.45,
...
function StartFight(fish)
    ...
    -- 体力 (旧鱼种有 stamina 字段；新鱼种按 diff 推算)
    local stamina = ft.stamina or (30 + (ft.diff or 3) * 14)
...
function DrawFight()
    ...
    if diagChartOn and #diagChartBuf > 1 then
        ...
        local dragY = chartY + chartH - (dragRatio * fight.lineStrength / yMax) * chartH
        nvgBeginPath(vg)
        ...
        nvgText(vg, chartX + chartW - 2, dragY - 2,
            string.format("阈值%.0fkg", fight.drag * fight.lineStrength))
...
function DrawCatchResult()
    ...
    if curFish then
        ...
        nvgText(vg, cx, cy + 5, "重量: " .. FormatWeight(curFish.weight), nil)
...
function DrawMinimap()
    ...
    local maxX = boat.x + viewHalf
    local minY = boat.y - viewHalf
    local maxY = boat.y + viewHalf
    local worldW = maxX - minX
    local worldH = maxY - minY

    -- ── 小地图尺寸和位置 ──
    local mmSize = 140                                -- 正方形边长
    local mmMargin = 12
    local mmX = screenW - mmSize - mmMargin            -- 右上角
    local mmY = mmMargin
    local cornerR = 8

    -- 等比缩放（世界 → 小地图）
    local scale = math.min(mmSize / worldW, mmSize / worldH)
    local drawW = worldW * scale
    local drawH = worldH * scale
    local ox = mmX + (mmSize - drawW) / 2
    local oy = mmY + (mmSize - drawH) / 2

    local function W2MM(wx, wy)
        return ox + (wx - minX) / worldW * drawW,
               oy + (wy - minY) / worldH * drawH
    end
    ...
    -- ── 水下等高线 ──
    DrawContourLines(minX, minY, worldW, worldH, 36, W2MM, false)
    ...
    for _, isl in pairs(allIslands) do
        ...
        if contour and #contour >= 3 then
            ...
            nvgClosePath(vg)
            nvgFillColor(vg, nvgRGBA(75, 145, 80, 180))
            nvgFill(vg)
        end
    end
