...
-- ============================================================================
-- 等高线/地形系统 (从 ContourMap.lua 加载)
-- ============================================================================
local ContourMap_ = require "ContourMap"
local DrawContourLines     = ContourMap_.DrawContourLines
local InvalidateContourCache = ContourMap_.InvalidateContourCache
local GetFishDepthLevel    = ContourMap_.GetFishDepthLevel
DEPTH_BANDS                = ContourMap_.DEPTH_BANDS  -- 全局，地图图例使用

-- ============================================================================
-- 岛屿礁石 (从 IslandRocks.lua 加载)
-- ============================================================================
local IslandRocks_ = require "IslandRocks"

-- ============================================================================
-- 海浪精灵 (从 WaveSprites.lua 加载)
-- ============================================================================
local WaveSprites_ = require "WaveSprites"

-- ============================================================================
-- 天气雨效 (从 WeatherRain.lua 加载)
-- ============================================================================
local WeatherRain_ = require "WeatherRain"
...
-- ============================================================================
-- 游戏数据 (从 GameData.lua 加载)
-- ============================================================================
local GameData_ = require "GameData"
local CFG              = GameData_.CFG
local ROD_TYPES        = GameData_.ROD_TYPES
local REEL_TYPES       = GameData_.REEL_TYPES
local FISH_TYPES       = GameData_.FISH_TYPES
local FLOOR_FISH_TYPES = GameData_.FLOOR_FISH_TYPES
local ROD_MIN_FIGHT_WEIGHT = GameData_.ROD_MIN_FIGHT_WEIGHT
GameData_ = nil  -- 释放引用，不占用 top-level local 以外的内存
...
-- 海浪精灵贴图（Start 中加载，注入 WaveSprites 模块）
...
-- ============================================================================
-- 地形工具 (从 ContourMap.lua 加载)
-- ============================================================================
local BuildIslandBumps = ContourMap_.BuildIslandBumps
local BuildDepthMap    = ContourMap_.BuildDepthMap
local GetDepthBandAt   = ContourMap_.GetDepthBandAt
local DEPTH_MAP_BOUNDS = ContourMap_.DEPTH_MAP_BOUNDS
ContourMap_ = nil  -- 释放引用
...
function Start()
    ...
    WaveSprites_.SetImages(waveImages)
...
function PickRandomFish()
    ...
    local weatherIdx = (WeatherRain_.GetMode() == "sunny") and 1 or 2    -- 1=晴天 2=雨天
...
function HandleUpdate(eventType, eventData)
    ...
    WeatherRain_.Update(dt, screenW, screenH)
    ...
    -- 海浪精灵 (全状态更新)
    WaveSprites_.Update(dt, camX, camY)
...
local function RunFishSimulation(n)
    ...
    local weatherLabel = (WeatherRain_.GetMode() == "sunny") and "晴天" or "雨天"
...
local function DrawAllIslandRocks()
    IslandRocks_.DrawAll(vg, WorldToScreen, screenW, screenH)
end

local function DrawWaveSprites()
    WaveSprites_.Draw(vg, WorldToScreen, screenW, screenH)
end
...
function HandleRender(eventType, eventData)
    ...
    if STATE ~= "menu" then
        DrawSkyOverlay()
        WeatherRain_.Draw(vg, screenW, screenH)
    end
...
function DrawMinimap()
    ...
    -- ── 等高线（实时绘制） ──
    DrawContourLines(minX, minY, worldW, worldH, 18, W2MM, true, vg)
...
function DrawMap()
    ...
    -- ── 水下等高线 ──────────────────────────────────────────────────────
    DrawContourLines(minX, minY, worldW, worldH, 80, W2M, true, vg)
